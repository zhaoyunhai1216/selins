...
public class AppsOptions {
    ...
    public static void createResources(String appID, byte[] b) throws IOException {
        String url = Configuration.getInstance().getString(Environment.APPSTORE_DIR) + "/" + appID + ".zip";
     ...
     * @throws IOException
     */
    public static boolean createZkResources(String appID, String clazz, String jvmOpts, int numWorkers, String category) {
        String zkDir = Configuration.getInstance().getString(Environment.ZK_ROOT_DIR) + "/applications/" + appID;
        AppResource resource = new AppResource(appID, UtilCommons.getAppName(clazz), jvmOpts, clazz, numWorkers, 0, category);
        return ZkUtils.create(ZkCurator.getInstance().getZkCurator(), zkDir, resource.toString().getBytes(), CreateMode.PERSISTENT);
    }
